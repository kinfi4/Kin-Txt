...
const fetchReport = (reportId) => {
    ...
    return axios
        .get(STATISTICS_SERVICE_URL + `/reports/${reportId}`, {
            headers: {
...
export const setComparisonReports =
    (firstReportId, secondReportId) => (dispatch) => {
...
export let comparisonReducer = (state = initialState, action) => {
    switch (action.type) {
        ...
        case SET_COMPARISON_REPORTS:
            return {
                firstReport: action.firstReport,
                secondReport: action.secondReport,
                reportsAreLoading: false,
            };
