...
import {
    downloadFile,
    transformLargeNumberToReadable,
} from "../../../../../../utils/utils";
                ...
                <div className={visualizationCss.exportOptions}>
                    <div
                        onClick={() => {
                            downloadFile(
                                STATISTICS_SERVICE_URL +
                                    `/reports-data/${report.reportId}?export_type=csv`,
                                "csv"
                            );
                        }}
                    >
                    ...
                    <div
                        onClick={() => {
                            downloadFile(
                                STATISTICS_SERVICE_URL +
                                    `/reports-data/${report.reportId}?export_type=json`,
                                "json"
                            );
                        }}
                    >
                        <VscJson style={{marginRight: "5px"}} />{" "}
                        <span style={{fontSize: "15px"}}>JSON</span>
                    </div>
                </div>
            );
        }
