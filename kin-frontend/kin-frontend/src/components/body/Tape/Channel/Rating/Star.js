...
import {
    changeCurrentRating,
    rateChannel,
} from "../../../../../redux/reducers/ratingReducer";
...
const Star = (props) => {
    ...
    if (props.currentRating) {
        ...
        starActiveClass =
            props.index <= Math.round(props.averageRating) ? s.active : "";
    }

    return (
        <div
            onClick={() => {
                props.rateChannel(props.link, props.index);
            }}
            className={starActiveClass}
            onMouseEnter={() => {
                props.changeCurrentRating(props.index);
            }}
            onMouseLeave={() => {
...
let mapDispatchToProps = (dispatch) => {
    return {
        rateChannel: (channelLink, rating) =>
            dispatch(rateChannel(channelLink, rating)),
