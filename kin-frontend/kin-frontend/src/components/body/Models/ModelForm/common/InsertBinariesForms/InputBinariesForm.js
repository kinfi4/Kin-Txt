...
const InputBinariesForm = ({
    ...
    if (!validatingUploadedTokenizerFiles && tokenizerFileUploadProgress === 100) {
    ...
    const [loadingTheModelFileIntoBrowser, setLoadingModel] = React.useState(false);
    const [loadingTheFileIntoBrowser, setLoadingTokenizer] = React.useState(false);
    ...
    return (
        <div className={styles.inputFilesContainer}>
            <div className={styles.inputFileContainer}>
                ...
                <input type="file" id="file" className={styles.fileInput} onChange={handleFileChange(onModelFileChange, setLoadingModel)} />
                <label htmlFor="file" className={`${styles.fileLabel} ${modelIsSet ? styles.inputFileSelected : ''}`}>
                    {
                        loadingTheModelFileIntoBrowser ? <Spinner /> :
                            modelIsLoading ? <>
                ...
                {
                    modelIsLoading ? <div className={styles.animatedProgressBar} >
                        <ProgressBar
            ...
            <div className={styles.inputFileContainer}>
                ...
                <input type="file" id="file" className={styles.fileInput} onChange={handleFileChange(onTokenizerFileChange, setLoadingTokenizer)} />
                <label htmlFor="file" className={`${styles.fileLabel} ${tokenizerIsSet ? styles.inputFileSelected : ''}`}>
                    {
                        loadingTheFileIntoBrowser ? <Spinner /> :
                            tokenizerIsLoading ? <>
                                <div style={{display: "flex", justifyContent: "space-between", alignItems: "center", width: "100%"}}>
                ...
                {
                    tokenizerIsLoading ? <div className={styles.animatedProgressBar} >
                        <ProgressBar
